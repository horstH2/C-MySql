 ...
 * Urheber: Horst-Christian Heinke, Magdeburg 24.06.2020
...
namespace OwnMySqlDB {
	class CsharpMySql {
		...
		#endregion 3. Getter
		#region 4. Methoden (Instanzmethoden)
...
#region 4.3. Datentabelle reorganisieren
/// <summary>
/// Entfernen einer Datentabelle aus dem Datenset
/// </summary>
public void removeTableInDataset() {
	dataSet.Tables.Remove(tableName);
}
/// <summary>
/// Fügt dem Datenset eine Tabelle hinzu
/// </summary>
public void addTableToDataset() {
	// Hinzufügen der Tabelle
	dataSet.Tables.Add(tableName);
	// Füllen mit aktualisierten Daten
	dataAdapter.Fill(dataSet.Tables[tableName]);
	// Binden der hinzugefügten Tabelle
	bindingSource.DataSource = dataSet.Tables[tableName];
}
		#endregion 4.3. Datentabelle reorganisieren
		#region 4.4. Nachschletabelle synchronisieren
		/// <summary>
		...
		/// mithilfe der der Komponente "DtaGridView" erfolgen.
		/// </summary>
		/// <param name="mainDataGrid"></param> Name der Hauptdatentabelle (DataGridView)
		/// <param name="subIdName"></param> Name des Identifikationsschlüssels der Subtabelle
		/// beziehungsweiese der Lookup-Tabelle
		public void showSubTableSynchronous(DataGridView mainDataGrid, string subIdName)
		{
			// aktuelle Zeile
			int aktuelleZeile = 0;
			try
			{  //Leere Tabellen sollten nicht zu einem Fehler führen
				...
				bindingSource.Position = bindingSource.Find(subIdName, id);
			}
			catch (Exception ex) { aktuelleZeile = 0; }
		}
		#endregion 4.4. Nachschletabelle synchronisieren
		#endregion 4. Methoden (Instanzmethoden)
	}
}
